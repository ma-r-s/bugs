...
<article class="space-y-10">
	<nav class="font-mono text-[11px] uppercase tracking-widest">
		<a href="/" class="text-ink-400 no-underline hover:text-cinnabar-600">← Catalog</a>
	</nav>
	...
	<header class="paper-card rounded-sm p-6 sm:p-10">
		...
		<h2
			class="mt-3 font-display text-3xl font-semibold leading-[1.1] text-ink-900 sm:text-5xl"
		>
			{bug.name}
		</h2>
		...
		<p
			class="mt-6 font-serif text-lg italic leading-snug text-ink-700 sm:text-xl"
		>
			{bug.tagline}
		</p>
		...
		>
			...
			<div class="flex justify-between gap-4 sm:block">
				<dt>Habitat</dt>
				<dd class="text-right text-ink-700 sm:mt-1 sm:text-left sm:normal-case sm:tracking-normal">
					{bug.habitat}
				</dd>
			</div>
			<div class="flex justify-between gap-4 sm:block">
	...
	{#if body}
		<section class="prose-bug mx-auto max-w-2xl font-serif text-[17px] leading-relaxed text-ink-700">
			{@render body()}
		</section>
	{/if}
	...
	{#if sources}
		...
		>
			<p class="mb-3 font-mono text-[11px] uppercase tracking-widest text-ink-400">
				Field Notes &amp; Sources
			</p>
			{@render sources()}
		</aside>
